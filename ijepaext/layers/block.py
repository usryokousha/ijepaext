...
from .attention import Attention, MemEffAttention, CrossAttention, MemEffCrossAttention
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x: Tensor, context: Tensor) -> Tensor:
        ...
        def cross_attn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(self.cross_attn(self.norm2(x), context))

        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls3(self.mlp(self.norm3(x)))
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + self.drop_path(cross_attn_residual_func(x))
